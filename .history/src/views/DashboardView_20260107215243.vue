            ...
            <div class="bg-white shadow rounded-lg p-4">
              <div class="flex items-center justify-between mb-4">
                ...
                <div class="flex items-center gap-2">
                  ...
                  <input v-model="search" placeholder="Search order or customer"
              ...
              <ul class="space-y-3 text-sm text-gray-600">
                <li v-for="(a, i) in recentActivity" :key="i">
                  <div class="flex items-start gap-3">
                    <div class="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center text-xs">{{ a.icon }}
                    </div>
                    <div>
...
onMounted(async () => {
  try {
    ...
    stats.value = {
      revenue: res.data.monthlyRevenue,
      orders: res.data.totalOrders,
      activeUsers: res.data.activeUsers,
      redeemed: res.data.rewardsRedeemed,

      // temp trends (can be replaced later with real analytics)
      revenueTrend: [800, 1100, 1250, 1000, 1400, 1700, res.data.monthlyRevenue],
      ordersTrend: [30, 40, 45, 35, 50, 60, res.data.totalOrders],
      usersTrend: [1000, 1020, 1100, 1150, 1170, 1200, res.data.activeUsers],
      redeemedTrend: [5, 8, 6, 7, 10, 12, res.data.rewardsRedeemed]
    }
  } catch (err) {
...
const recentActivity = [
  ...
  { icon: '👤', title: 'New user signed up', time: '1 day ago' }
]
