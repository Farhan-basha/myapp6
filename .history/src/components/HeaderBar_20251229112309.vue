  ...
  <header class="bg-white shadow-sm">
    <div class="max-w-6xl mx-auto px-6 py-6 flex items-center justify-between">
      ...
      <nav class="hidden md:flex gap-8 text-sm text-gray-700 font-medium">
        <RouterLink class="hover:text-olive cursor-pointer hover:bg-gray-300" to="/">Home</RouterLink>
...
// ✅ Reactive state from Pinia (THIS is the fix)
const user = computed(() => auth.user)
const isLoggedIn = computed(() => auth.isLoggedIn)
const isAdmin = computed(() => auth.user?.role === 'ROLE_ADMIN')
