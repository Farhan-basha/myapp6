...
import { modal, closeModal, openSignup, openForgot } from '../store/ui.js' // <-- ensure path and .js
...
const submit = async () => {
    ...
    try {
        // mock login flow (replace with real API call)
        const mockToken = 'mock-jwt-token'
        localStorage.setItem('dp_token', mockToken)
        close()
        alert('Logged in (mock). Token saved to localStorage.')
    } catch (e) {
